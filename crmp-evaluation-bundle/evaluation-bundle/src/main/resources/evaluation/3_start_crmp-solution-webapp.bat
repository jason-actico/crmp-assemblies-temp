...
rem --- 'crmp-solution-webapp' below this script directory and re-
...
if not exist "crmp-solution-webapp" (
    echo Detected first start; about to unzip and launch ACTICO CRMP Solution Web Application...
    echo.

    mkdir crmp-solution-webapp
    pushd crmp-solution-webapp
       jar --verbose --extract --file ..\..\install\financial-spreading-platform-webapp*.zip
    echo.
    popd
    
    pushd crmp-solution-webapp\config\lib
       copy ..\..\..\..\misc\crmp-dbmodel-*.jar .
    popd

    mkdir crmp-solution-webapp\import
	pushd crmp-solution-webapp\import
...
set "CRMP_SOLUTION_WEBAPP_CMD=cd %SCRIPT_DIR%crmp-solution-webapp\bin\windows"
